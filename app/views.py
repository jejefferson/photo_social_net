...
from retrieve2 import CacheRData
from forms import LoginForm, RealLogin, PostForm, SignForm, SubmitForm, ProfileChange, SendMessage, UploadForm,\
...
try:
	...
	def dump_last_online_date(signum):
			recent_online_users = [usr for usr,lasttime in ONLINE_SOCIAL_TABLE.items() if \
				lasttime + datetime.timedelta(minutes=app.config['TIMEOUT_SOCIAL'], seconds=10)< datetime.datetime.utcnow()]
			if recent_online_users:
				for user in recent_online_users:
					app.logger.info('I sew recent online user: %s at %s' % (user, ONLINE_SOCIAL_TABLE[user].strftime('%H:%M:%S')))
		...
		if user:
			if password == user.password:
				...
				if form.remember_me.data:
					s = itsdangerous.Signer(app.secret_key, salt = 'солюжечка')
					signed_nickname = s.sign(nickname)
					response.set_cookie('nickname', value = signed_nickname, expires = datetime.datetime.utcnow()+\
						datetime.timedelta(hours=app.config['COOKIE_EXPIRATION']))
				return response
			else:
...
def upload_file():
	...
	files = models.UploadedFile.query.filter_by(access = 'file_exchange').order_by(desc(models.UploadedFile.upload_data)).limit(10).all()
...
def usermodprofile():
	...
	if form.validate_on_submit():
		...
		gender = [(0, u'male'), (1, u'female'), (2, u'trans'), (3, u'cyberpunk'), (4, u'noselect')][form.user_gender.data][1]
...
def userprofile(nick, message_id):
	...
	friend = True if nick and models.User.query.filter_by(nickname = session.get('nickname')).first().is_friend(user) else False
...
def profile_messages():
	...
	page = request.args.get('page', '')
	messages_q = posted.union(recieved).order_by(desc(models.Message.timestamp)).options(joinedload('user')).options(eagerload('comments'))
	messages_count = messages_q.count()
	pages_count = messages_count/20
	page = _validate_page(page, pages_count)
...
def addfriend(nick):
	...
	if not u:
		...
		return redirect(redirect_url())
	db.session.add(u)
	message = models.Message(body = u'Service message. User %s added you to friends.\nСервисное сообщение. Пользователь %s добавил вас в друзья.' % (me.nickname, me.nickname), msg_author = me.nickname, msg_dest = user.nickname, msg_type = 'private',\
				timestamp = datetime.datetime.utcnow(), del_from_author = True, del_from_dest = False)
	db.session.add(message)
	db.session.commit()
...
def photo(id, photonum):
	...
	return render_template('photo.html', nickname = user.nickname, gallery = gallery, photo = photo,\
			form = form, photonum = photonum, comments = comments, guest = guest, friend = friend, title = 'photo view')
...
def _delfile(id):
	...
	if message and message.msg_dest != user.nickname and message.msg_author != user.nickname:
		flash(gettext('Hackors not allowed'))
		return redirect(redirect_url())
	_remove_from_disk(file.filename)
...
def test():
	...
	if messages:
		...
		for message in messages.all():
			result += '<span id="message">['+str(message.timestamp)+']<font color="#9C0064">&lt;'+message.msg_author+'&gt;</font><span style="white-space: pre-wrap">'+message.body+'\n'+'</span><span style="display: none;" id="msg_id">|'+str(message.msg_id)+'</span></span>'
...
def search_images():
	...
	camera_list = [f[0] for f in models.Exif.query.filter(models.Exif.model != 'None').with_entities(models.Exif.model).distinct().all()]
	camera_list.insert(0, 'select')
	lens_list = [f[0] for f in models.Exif.query.filter(models.Exif.lens_model != 'None').with_entities(models.Exif.lens_model).distinct().all()]
...
def news():
	...
	comments_to_my_messages = models.Message.query.filter_by(msg_type='blog').outerjoin(parent_message, parent_message.\
			parent_id==models.Message.msg_id).filter(models.Message.msg_author == nickname).outerjoin(com, com.parent_id == models.Message.msg_id).with_entities(com)
	comments_to_other_messages = models.Message.query.filter_by(msg_type='blog').outerjoin(parent_message, parent_message.\
			parent_id==models.Message.msg_id).filter(parent_message.msg_author == nickname).filter(models.Message.\
			msg_dest != nickname).outerjoin(com, com.parent_id == models.Message.msg_id).with_entities(com)
...
def group(name, message_id):
	...
	if group.tags:
		...
		for tag in group.tags[1:]:
			sub_messages = sub_messages.union(models.Message.query.filter(models.Message.tags.any(models.Tag.entity==tag.entity)))
	...
	return render_template('group.html', owner=owner, group=group,members=members, subscription=subscription,
			nickname=user.nickname, messages=all_messages, form=form, form2=form2, blog=True, title=group.name)
